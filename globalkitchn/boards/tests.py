...
class BoardTopicsTests (TestCase):
    ...
    def test_board_topics_view_success_stat_code(self):
        url = reverse('board_topics', kwargs={'pk': 1})
        response = self.client.get(url)
        self.assertEquals(response.status_code, 200)

    def test_board_topics_view_not_found_stat_code(self):
...
class NewTopicTests (TestCase):
    def setUp(self):
        ...
        User.objects.create_user(
            username='mckenners', email='mckenners@example.com', password='bali')

    def test_new_topic_view_success_stat_code(self):
        url = reverse('new_topic', kwargs={'pk': 1})
        response = self.client.get(url)
        self.assertEquals(response.status_code, 200)

    def test_new_topic_view_not_found_stat_code(self):
